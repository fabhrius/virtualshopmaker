    ...
    if ($('.wp-block-ultimate-post-advanced-search').length) {
        ...
        const handleSetPosition = ( el, newBlock = false) => {
            ...
            if (popupType == "popup") {
                ...
                const elementPosition = posSelector.offset();
                const contentPosition = popupposition == 'right' ? (elementPosition?.left > $(`body > .ultp-block-${blockId}`).width()) : (elementPosition?.left > $(`body > .ultp-block-${blockId}`).width());
                
                return $(`body > .ultp-block-${blockId}`).css({ "top" : `${elementPosition?.top +  posSelector.outerHeight()}px`, 'right' : `${ contentPosition ? ($(document).width() - elementPosition?.left) - posSelector.outerWidth() + 'px' : 'unset'}`, 'left' : `${ contentPosition ? 'auto' : (elementPosition?.left + (popupposition == 'right' ? 10 : 0)) + 'px'}`});
            }            
        }
    }
